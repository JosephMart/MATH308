...
\begin{enumerate}[I.]
  ...
  \begin{enumerate}[a)]
    ...
    \item \textbf{Numerical solution.} \\
      ...
      The exact solution for $y(0)=-1.5$ found using Matlab is given by
      \begin{align*}
        y=-\dfrac{1}{x+ \frac{2}{3}}
      \end{align*}
      ...
      The graph of the results of Euler's method, with $h=0.1$ and $h=0.01$, is compared to the exact solution below
            ...
            \begin{figure}[H]
              \begin{center}
                \includegraphics[scale=.9]{213.png}
                \caption{With $y(0) = -1.5$}
              \end{center}
            \end{figure}
            
    \item  \textbf{Error.} Error, as a function of h. We fix the initial condition, and the interval [0,1], and then compute the error (defined as the maximum of the absolute value of the pointwise error over the interval) versus the step size h. The initial condition being analyzed is for $y(0) = -1.5$
    
    \begin{table}[H]
      \caption{Error Analysis of $y = -\dfrac{1}{x+\frac{2}{3}}$}
  ...
  \lstinputlisting[language=matlab,caption=euler.m]{euler.m}
  \lstinputlisting[language=matlab,caption=rhs.m]{rhs.m}
  \lstinputlisting[language=matlab,caption=rhs2.m]{rhs2.m}
  \lstinputlisting[language=matlab,caption=main.m]{main.m}
\end{enumerate}
